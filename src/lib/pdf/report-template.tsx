...
function StatusBadge({ status }: { status: string }) {
  ...
  const bg = status === "valid" ? `${C.valid}15` : status === "expiring" ? `${C.expiring}15` : status === "expired" ? `${C.expired}15` : `${C.ink}10`;
...
export function ComplianceReport({ data, tier }: { data: ReportData; tier?: "basic" | "audit" | "white_label" }) {
    ...
    <View style={styles.section}>
      ...
      <View style={styles.executiveSummary}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <View>
            <Text style={styles.summaryScore}>{complianceScore}%</Text>
  ...
  const registerContent = !isBasic ? (
    <View style={styles.section} break>
      <Text style={styles.sectionTitle}>Staff Credential Register</Text>
  ...
  const summaryContent = (
    <View style={styles.section} break>
      <Text style={styles.sectionTitle}>Credential Status Summary</Text>
